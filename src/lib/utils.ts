...
${promptsArrayString}
        ...
        resetPrompts: function() {
            GM_setValue('promptAmplifierEnhancedPrompts_v1', this.initialPrompts.map(p => ({...p, id: p.id || this.generateId()})));
        ...
        renderPromptListModal: function(promptType, modalTitlePrefix) {
            ...
            if (fP.length === 0) {
                ...
                fP.forEach(p => {
                    ...
                    lIW.addEventListener('click', (e) => { this.copyToClipboard(p.content, p.title, 'prompt-list-modal'); });
                });
            }
