...
export function generateTampermonkeyScript(prompts: Prompt[]): string {
  ...
  const scriptBody = [
    ...
    '        },',
    '',
    '        escapeForTemplateLiteral: function escapeForTemplateLiteral(str) {',
    ...
    "            navigator.clipboard.writeText(text).then(() => {",
    "                let message = 'Copied `'.concat(titleForNotification, '` to clipboard!');",
    '                if (isTemplate) {',
    "                    message = 'Template `'.concat(titleForNotification, '` copied.\\nPaste into initialPrompts array and edit.');",
    '                }',
    ...
    "            mainRow.className = 'prompt-helper-main-row';",
    '            mainButtonContainer.appendChild(mainRow);',
    '',
    "            const quickActionsRow = document.createElement('div');",
    "            quickActionsRow.className = 'prompt-helper-quick-actions-row';",
    '            mainButtonContainer.appendChild(quickActionsRow);',
    '',
    "            const moveHandle = document.createElement('button');",
    "            moveHandle.id = 'prompt-helper-move-handle';",
    "            moveHandle.innerHTML = '&#10021;';",
    "            moveHandle.title = 'Drag to move this panel';",
    '            mainRow.appendChild(moveHandle);',
    '',
    ...
    '            mainRow.appendChild(scriptInstructionsButton);',
    ...
    '            mainRow.appendChild(copyPageUrlButton);',
    ...
    '                    button.textContent = action.title.substring(0, 4);',
    '                    button.title = "Quick Action: ".concat(action.title, "\\n", "Copy: \\"", action.content, "\\"");',
    "                    button.className = 'prompt-helper-preset-button';",
    "                    button.addEventListener('click', (e) => {",
    '                        e.stopPropagation();',
    '                        this.copyToClipboard(action.content, action.title);',
    '                    });',
    '                    quickActionsRow.appendChild(button);',
    '                });',
    '            }',
    '        },',
    '',
    ...
    "                           lI.title = p.category.concat(' > ', p.title, '\\nClick to copy');",
    '                        } else {',
    "                           lI.title = p.title.concat('\\nClick to copy');",
    '                        }',
    ...
    '            const m = this.createModal(mId, modalTitlePrefix.concat(" List"), cD, footerButtons);',
    ...
    "            const mId = 'prompt-preview-modal-'.concat(String(prompt.id || '').replace(/[^a-zA-Z0-9-_]/g, ''));",
    "            const previewTitle = 'Preview: '.concat(String(prompt.title || ''));",
    ...
    "                    position: fixed; z-index: 9999; display: flex; flex-direction: column; gap: 4px;",
    ...
    "                }",
    "                .prompt-helper-main-row, .prompt-helper-quick-actions-row {",
    "                    display: flex;",
    "                    flex-wrap: wrap;",
    "                    gap: 6px;",
    "                    align-items: center;",
